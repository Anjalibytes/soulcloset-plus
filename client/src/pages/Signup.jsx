...
export default function Signup({ onSignup, authError }) {
  ...
  async function submit(e) {
    e.preventDefault();
    setLocalError("");
    if (!name.trim()) return setLocalError("Tell us what to call you.");
    ...
    setBusy(true);
    const ok = await onSignup({ name: name.trim(), email: email.trim(), password });
    setBusy(false);
    ...
    <div style={{ background: "#F6EFE4", minHeight: "100vh", color: "#33232E" }} className="flex items-center justify-center px-6">
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-lg p-7" style={{ background: "#FFFDF9", border: "1px solid #E4D8C4" }}>
          <h2 className="fraunces text-2xl mb-1" style={{ fontStyle: "italic" }}>Let's set up your closet.</h2>
          <p className="text-xs opacity-60 mb-6">Your password is hashed and never stored in plain text.</p>

          <form onSubmit={submit} className="space-y-4">
            <div>
              <label className="mono text-[10px] opacity-60 block mb-1">YOUR NAME</label>
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Anjali" className="w-full text-sm p-2.5 rounded border" style={{ borderColor: "#C9BBA0" }} />
            </div>
            <div>
            ...
            <div>
              ...
              <input value={password} onChange={(e) => setPassword(e.target.value)} placeholder="At least 6 characters" type="password" className="w-full text-sm p-2.5 rounded border" style={{ borderColor: "#C9BBA0" }} />
            </div>
            ...
            <button type="submit" disabled={busy} className="w-full py-2.5 rounded-full text-sm flex items-center justify-center gap-2" style={{ background: "#C4707A", color: "#FFFDF9" }}>
              {busy && <Loader2 className="animate-spin" size={14} />} Create my closet
            </button>
          </form>
